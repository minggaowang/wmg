...
<input type="${fieldType}" id="${status.expression}" name="${status.expression}"
       value="${stringStatusValue?html}" ${attributes}<@closeTag/>
</#macro>
...
<#macro formSelect path options attributes="">
<@bind path/>
<select id="${status.expression}" name="${status.expression}" ${attributes}>
    <#if options?is_hash>
        <#list options?keys as value>
            <#if value=="0">
                <option value="${value?html}"<@checkSelected "0"?html/>>${options["0"]?html}</option>
            <#else >
                <option value="${value?html}"<@checkSelected value?html/>>${options[value]?html}</option>
            </#if>
        </#list>
        <#else>
            <#list options as value>
...
<#--just for bootstrap-->
<#macro formCheckboxesBS path options separator checkboxAttr="" labelAttr="" labeli18n=true>
<@bind path/>
    <#list options?keys as value>
        <#assign id="${status.expression}${value_index}">
        <#assign isSelected = contains(status.actualValue?default([""]), value)>
    <label for="${id}" ${labelAttr}><input type="checkbox" id="${id}" name="${status.expression}"
                                           value="${value?html}"<#if isSelected>
                                           checked="checked"</#if> ${checkboxAttr}<@closeTag/>
        <#if labeli18n==true>
...
<#macro formRadioButtonsBS path options separator radioButtonAttr="" labelAttr="" labeli18n=true default="">
<@bind path/>
    <#list options?keys as value>
        <#assign id="${status.expression}${value_index}">
    <label for="${id}" ${labelAttr} ><input type="radio" id="${id}" name="${status.expression}"
                                            value="${value?html}"
        <#if stringStatusValue == value>
           checked="checked"
        <#elseif stringStatusValue==""&&default==value>
           checked="checked"
        </#if> ${radioButtonAttr}<@closeTag/>
    <#if labeli18n==true>
        <@message options[value]/>
            <#else>
            ${options[value]?html}
...
<#macro formArrayControl path separator attributes="" >
<@bind path/>
<#list 0..status.value?size-1 as index>
    <#nested path+"["+index+"]",index>
</#list>
</#macro>